...
### 2025-12-05 - カイゴジョブ・ジョブメドレー対応

#### カイゴジョブ (kaigoagent.com) スクレイピング対応
- **カイゴジョブ スクレイピングに新規対応**
- React SPA (Next.js) 対応の専用スクレイパー実装
- 47都道府県ID対応（prefecture_12000026=東京 等）
- 職種カテゴリによる自動分類:
  - `10100013` (介護事務・医療事務) → **事務**カテゴリ
  - `10100008` (送迎運転手・ケアドライバー) → **ドライバー**カテゴリ
  - その他 → **介護**カテゴリ
- `__NEXT_DATA__`解析による詳細情報取得

#### ジョブメドレー (job-medley.com) スクレイピング対応
- **ジョブメドレー スクレイピングに新規対応**
- 職種カテゴリコード検索に対応:
  - `sr` = 営業
  - `hh` = 介護職/ヘルパー
  - `cm` = ケアマネジャー
  ...
  - `mc` = 医療事務
  - `cw` = 保育士
- 47都道府県対応（pref13=東京 等）
- `order=2`で新着順検索
- `__NEXT_DATA__`とJSON-LD解析による詳細情報取得

#### タウンワーク営業カテゴリ修正
- 営業検索を特定の職種コードに限定
- `omc=0011, 0012, 0013`の3カテゴリのみに絞り込み
- 雇用形態: `emp=01,02,03,05`（アルバイト・パート等）
